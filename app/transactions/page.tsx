...
export default function TransactionsPage() {
  ...
  const filtered = useMemo(() => {
    return expenses
      .filter(e => (q ? e.description.toLowerCase().includes(q.toLowerCase()) : true))
      .filter(e => (cat === 'all' ? true : e.categoryId === cat))
      .sort((a, b) => b.date.localeCompare(a.date))
  }, [expenses, q, cat])

  const lookup = useMemo(() => Object.fromEntries(categories.map(c => [c.id, c])), [categories])
    ...
    <main className="space-y-6">
      ...
      <section className="overflow-x-auto rounded-2xl border bg-white dark:bg-black">
        <table className="w-full min-w-[680px] text-sm">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-500 dark:text-gray-400">No transactions</td></tr>
            )}
            {filtered.map(e => {
                ...
                <tr key={e.id} className="border-t">
                  ...
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center gap-2">
                      <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: cat?.color ?? '#64748b' }} />
                      {cat?.name ?? 'Other'}
                    </span>
                  </td>
